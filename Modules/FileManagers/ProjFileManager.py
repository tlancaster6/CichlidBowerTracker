...
class ProjFileManager:
	...
	def _downloadDirectory(self, directory):
		...
		output = subprocess.run(['rclone', 'copy', self.cloudMasterDir + tar_directory, self.localMasterDir, '--fast-list', '-vv', '--checkers', '40', '--transfers', '40', '--tpslimit', '10', '-P'],stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf-8')
		if os.path.exists(self.localMasterDir + tar_directory):
			...
			output = subprocess.run(['rclone', 'copy', self.cloudMasterDir + directory, self.localMasterDir + directory, '--fast-list', '-vv', '--checkers', '40', '--transfers', '40', '--tpslimit', '10', '-P'],stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf-8')
			print(output.stdout)
			if not os.path.exists(self.localMasterDir + directory):
