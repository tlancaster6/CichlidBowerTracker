...
class MLClusterPreparer:
	...
	def _prepareClips(self):
		...
		subprocess.run(['tar', '-xvf', self.projFileManager.localMasterDir + 'preppedClips.tar', '-C',
	...
	def _predictLabels(self):
		...
		with open(self.projFileManager.localTroubleshootingDir + 'command.out') as f:
			subprocess.run(outCommand, env=trainEnv, stdout=f, stderr=subprocess.STDOUT)
